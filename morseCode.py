...
def is_help_command(user_input):
    ...
    if user_input == 'H' or user_input.upper() == "HELP":
        result = True
    else:
        result = False
    return result
...
def is_validated_english_sentence(user_input):
    ...
    # ===Modify codes below=============
    # 조건에 따라 변환되어야 할 결과를 result 변수에 할당 또는 필요에 따라 자유로운 수정
    tempList = user_input.split('')
    result = True
    isContainingSpace = False
    isContainingAllowedChar = False
    SPECIAL_CHAR = ['_', '@', '#', '$', '%', '^', '&', '*', '(', ')', '-', '+', '=', '[', ']', '{', '}', '"', "'", ';', ':', '\\', '|', '`', '~']
    ALLOWED_CHAR = ['.', ',', '!', '?']
    for i in tempList:
        if i.isnumber():
        ...
        if i.isalpha():
            isContainingAllowedChar = True
        for j in ALLOWED_CHAR:
            if i == j:
                isContainingAllowedChar = True

    if not (isContainingAllowedChar and isContainingSpace):
        result = False
